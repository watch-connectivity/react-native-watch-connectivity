...
import * as watch from '../../lib/RNWatch';
...
const { StyleSheet, View, TextInput, LayoutAnimation, StatusBar } = ReactNative,
  { Component } = React;
...
export default class Root extends Component {
      ...
      .then(image => {
        ...
        if (!image.didCancel) {
          ...
          if (fileAPI && image.uri) {
            promise = watch.transferFile(image.uri);
          } else if (image.data) {
            promise = watch.sendMessageData(image.data);
          } else {
    ...
    if (text.trim().length) {
      ...
      watch.sendMessage({ text, timestamp }, (err, resp) => {
        if (!err) {
          ...
          const timeTakenToReply =
            new Date().getTime() - parseInt(resp.timestamp);
          this.configureNextAnimation();
          this.setState({
  ...
  render() {
    return (
      <View style={styles.container}>
        ...
        />
        {this.state.loading && (
          <Spinner type="Bounce" color={COLORS.orange} size={44} />
        )}
  ...
  receiveWatchState(err, watchState) {
    if (err) {
      console.error('Error receiving watch state', err);
    } else {
